...
- Visibility: a profile's lists are viewable if `isSelf || owner.isPublic || mutual-follow` (you each follow the other). The rule lives in **`lib/visibility.ts`** (`getFollowState` + `canViewList`) — use that helper, don't re-derive it per route. A **one-way** follow does NOT unlock a private list (following is instant/unapproved, so that would let anyone self-grant access). `GET /api/users/[username]/route.ts` returns `canViewList` (and gates `animeCounts` on it); `GET /api/users/[username]/anime` enforces the same rule (403 if private, not self, and not mutual) and flags each anime with `inMyList` (matched by `malId` or `normalizedName`+`season` against the viewer's library). The mobile client renders the private state purely from the API's `canViewList` flag. Note `followers`/`following` list routes are NOT privacy-gated.
